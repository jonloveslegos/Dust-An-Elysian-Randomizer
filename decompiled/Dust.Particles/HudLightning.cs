...
{
	internal class HudLightning : Particle
	{
		...
		public HudLightning(Vector2 loc, bool master, float lifeSpan, int _boltCount, int _boltLength, float _boltWidth, int l)
		{
		...
		public override void Update(float gameTime, Map map, ParticleManager pMan, Character[] c, int l)
		{
			for (int i = 1; i < this.boltNodes.Length; i++)
			{
				if (this.boltNodes[i] == Vector2.Zero && this.boltNodes[i - 1] != Vector2.Zero && Rand.GetRandomInt(0, 10) < 5)
				{
					...
					{
						pMan.AddHudLightning(this.boltNodes[i], master: false, this.frame, Math.Max(this.boltNodes.Length / 2, 2), this.boltLength, this.boltWidth, l);
					}
				}
			}
		...
		public override void Draw(SpriteBatch sprite, Texture2D[] particlesTex, float worldScale, int l)
		{
			if (base.flag == 0)
			{
				sprite.Draw(particlesTex[2], this.boltNodes[0], new Rectangle(964, 3960, 504, 136), Color.White * this.frame * Rand.GetRandomFloat(1f, 4f), 0f, new Vector2(250f, 70f), 1f, SpriteEffects.FlipHorizontally, 0f);
			}
			...
			for (int i = 0; i < this.boltNodes.Length - 1; i++)
			{
				if (this.boltNodes[i + 1] != Vector2.Zero)
				{
					float num = ((float)(this.boltNodes.Length - i) / (float)this.boltNodes.Length + 0.25f) * 2f * Game1.hiDefScaleOffset;
					float num2 = (this.boltNodes[i + 1] - this.boltNodes[i]).Length();
					float num3 = GlobalFunctions.GetAngle(this.boltNodes[i], this.boltNodes[i + 1]) + 1.57f;
					Rectangle value2;
					...
					sprite.Draw(particlesTex[2], this.boltNodes[i], value2, Color.White * this.frame * 4f, num3, new Vector2(20f, 10f), new Vector2(Rand.GetRandomFloat(0.8f, 1f) * this.boltWidth * num, num2 / 110f), (this.boltType[i] >= 4) ? SpriteEffects.FlipVertically : SpriteEffects.None, 0f);
					if (Rand.GetRandomInt(0, 30) == 0)
					{
						sprite.Draw(particlesTex[2], (this.boltNodes[i] + this.boltNodes[i + 1]) / 2f, value, Color.White * this.frame * 4f, num3 + 1.57f, new Vector2(248f, 75f), 0.2f * Game1.hiDefScaleOffset, SpriteEffects.None, 1f);
					}
				}
			}
		}
